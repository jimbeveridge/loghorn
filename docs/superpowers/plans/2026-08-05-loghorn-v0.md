...
```
Expected: creates `go.mod` with `module loghorn` and `go 1.22` (edit the `go` line to `1.22` if the local toolchain writes a higher patch line). Note: adding bubbletea later raises this to `go 1.24` via `go mod tidy` — that's expected, not a mistake.
