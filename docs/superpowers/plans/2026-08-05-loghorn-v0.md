# loghorn v0 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship the loghorn v0 walking skeleton — pipe GCP Cloud Run logs (LogEntry JSON or raw text) into a keyboard-first TUI that shows only important lines with leading context, opens full colorized JSON on demand, fires coalesced desktop alerts, and also runs headless as a stdin→stdout filter.
...
**Tech Stack:** Go 1.24 (effective floor — see Global Constraints), Bubble Tea + Lip Gloss + Bubbles (TUI), gen2brain/beeep (desktop notifications), Go standard library `testing` for tests.

## Global Constraints

- Go version floor: originally targeted **Go 1.22**, but the shipped floor is **Go 1.24** — bubbletea's transitive deps (`colorprofile`, `x/ansi`, `x/cellbuf`) require it and `go mod tidy` raises the `go` directive automatically. `go.mod` module path: **`github.com/jimbeveridge/loghorn`** (originally the bare local path `loghorn`, renamed to the canonical URL once the repo existed — treat every import as `github.com/jimbeveridge/loghorn/internal/...`).
...
Run:
```bash
cd ~/code/loghorn && go mod init loghorn
```
Expected: creates `go.mod` with `module github.com/jimbeveridge/loghorn` and `go 1.22` (edit the `go` line to `1.22` if the local toolchain writes a higher patch line). Note: adding bubbletea later raises this to `go 1.24` via `go mod tidy` — that's expected, not a mistake.
...
func (c *Coalescer) Observe(e entry.Entry) (bool, string, string) {
	...
	if !c.started || t.Sub(c.windowOpen) >= c.cooldown {
		title := "loghorn: important log activity"
		var body string
		if c.suppressed > 0 {
...
func (m Model) statusBar() string {
	...
	return statusStyle.Render(fmt.Sprintf(
		" loghorn · %s · %d shown · j/k move · space %s · enter inspect · q quit",
