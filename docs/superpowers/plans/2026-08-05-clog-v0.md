...
- Deferred to v1 (do NOT build in v0): multi-line stack-trace grouping, general JSONPath field resolution, the QBE query grid, interactive search, search-to-rule, a config file. v0 configuration is CLI flags only.
...
- Produces:
  ...
  - `type Entry struct { Raw []byte; Format Format; Timestamp time.Time; Severity Severity; HTTPStatus int; Message string; JSON map[string]any; Malformed bool; Important bool }`.
...
- Produces:
  ...
  - `type LogEntryAdapter struct{}` implementing `Adapter`. `Detect` returns true when the line trims to a JSON object (first non-space byte is `{`). `Parse` unmarshals into `map[string]any` and extracts `severity`, `httpRequest.status`, `timestamp` (RFC3339), and a display `message` (preferring `textPayload`, then `jsonPayload.message`, then top-level `message`, else compact JSON). Returns an error on invalid JSON.
