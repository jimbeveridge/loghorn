...
- **Colours that read on any background**: every colour was a fixed 256-colour index picked for
  ...
  it will be drawn with. The attention role (help keys, JSON numbers, bar notes) targets 4.5:1
  like every other role, except on a light background, where it drops to 3:1 and goes bold —
  WCAG's large-text threshold, applied to short bold tokens as a deliberate trade, since 4.5:1
  forced a near-black brown on the user's `#cee8be` terminal with almost no differentiation. Dark
  backgrounds are unaffected, including ones far from black such as Nord's `#3b4252`. On a light
  background the log list's non-failure rows also drop their dim-grey colour entirely, falling
  back to the terminal's own (typically black) text, after the same user found the dim grey
  washed out and asked for a strong black there; dark backgrounds keep the dim role's grey. See
